...
StatusDetection::StatusDetection():status_detect_{vitis::ai::YOLOv3::create(modelYolo)}
...
StatusDetection::run(const cv::Mat &input_image) {
  ...
  if(countWarning >= 1 && countWarning <= 90){
    countWarning ++;
    cv::putText(image, "You seem drowsy.Please consider taking a break", cv::Point(180, 50), cv::FONT_HERSHEY_COMPLEX, 1, cv::Scalar(0, 0, 255), 2);
  }
  else {
    countWarning = 0;
    int label;
    ...
    for (const auto bbox : status_detect_results.bboxes) {
      //int label = bbox.label;
      if(maxConfidence < bbox.score){
        maxConfidence = bbox.score;
        label = bbox.label;
      }
    }
